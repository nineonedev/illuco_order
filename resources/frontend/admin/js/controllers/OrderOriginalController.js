...
export default class OrderOriginalController extends Controller {
    ...
    _;
    /* ===== 테이블 폼 초기화 (행 단위 수정/삭제) ===== */
    _initForms() {
        // 테이블 바깥에 이벤트 위임: 동적으로 행이 바뀌어도 동작
        const tableArea = document.querySelector(".no-page-index-table-outer");
        if (!tableArea) return;

        // 클릭으로 submit 버튼만 캐치 (수정/삭제)
        tableArea.addEventListener("click", async (evt) => {
            const btn = /** @type {HTMLButtonElement|null} */ (
                evt.target.closest("button[type='submit']")
            );
            if (!btn) return;

            // 연결된 form 찾기: 우선 button@form, 없으면 가장 가까운 form
            ...
            // 삭제 확인
            const methodOnBtn = (btn.getAttribute("value") || "").toUpperCase();
            const nameOnBtn = btn.getAttribute("name") || "";
            const isDelete =
                nameOnBtn === "_method" && methodOnBtn === "DELETE";
            if (isDelete) {
                const ok = window.confirm(
                    "삭제 후 복구가 어렵습니다. 진행할까요?"
                );
                if (!ok) return;
            }
            ...
            try {
                // URL/메서드 결정 (button.formAction 우선, 없으면 form.action)
                const url =
                    btn.formAction ||
                    form.getAttribute("action") ||
                    window.location.href;

                // _method 판단: 1) 버튼(name=_method) 2) form의 hidden _method 3) 기본 PUT (update)
                let methodOverride = null;
                if (nameOnBtn === "_method" && methodOnBtn) {
                    methodOverride = methodOnBtn; // 버튼으로 DELETE 등 덮어쓰기
                } else {
                    const hidden = form.querySelector("input[name='_method']");
                    methodOverride = hidden
                        ? (hidden.value || "").toUpperCase()
                        : "PUT";
                }

                // 서버로 보낼 FormData 구성 (버튼 name/value도 포함하여 네이티브 제출과 동일하게)
                const fd = this._buildFormData(form, btn);

                // Ajax 요청: 백엔드가 라우트에서 _method를 해석하므로 HTTP는 POST로 통일
                const ajax = new Ajax(true);
                const result = await ajax.post(url, fd);

                if (!result || result.success === false) {
                    const msg = result?.message || "요청 처리에 실패했습니다.";
                    alert(msg);
                    return;
                }

                // 성공 후 처리: 간단히 새로고침(가장 확실)
                // 필요하면 여기서 특정 행만 부분 갱신하도록 개선 가능
                if (isDelete) {
        ...
        // 폼 자체의 submit도 Ajax로 가로채기 (혹시 직접 submit될 경우)
        document.querySelectorAll("form[id^='item-form-']").forEach((form) => {
            form.addEventListener("submit", (evt) => {
                // 위의 버튼 핸들러에서 처리하므로 여기서는 기본 동작 방지만
                evt.preventDefault();
            });
        });
    }

    /* ===== 폼 직렬화 (네이티브 제출과 동일하게 구성) ===== */
    _buildFormData(form, submitButton) {
        const fd = new FormData(form);

        // 버튼에 name/value가 있으면 네이티브처럼 포함시켜야 한다.
        if (submitButton && submitButton.name) {
            fd.append(submitButton.name, submitButton.value ?? "");
        }

        // CSRF 토큰이 meta나 hidden으로 존재할 경우 안전하게 보정
        if (!fd.has("_token")) {
            ...
            if (tokenInput && tokenInput.value) {
                ...
                if (meta && meta.content) fd.append("_token", meta.content);
            }
        }

        // order_id가 행 폼에 없다면 상단 폼/DOM에서 찾아서 주입 (보안 검증에 도움)
        if (!fd.has("order_id")) {
            const orderInput = document.querySelector("input[name='order_id']");
            if (orderInput && orderInput.value) {
                fd.append("order_id", orderInput.value);
            }
        }
    ...
    async _loadAllData() {
        ...
        try {
            ...
            OrderOriginalController.attributes = attrResult.data?.loupe
                ? attrResult.data
                : {};
            OrderOriginalController.setGroupItems =
                setGroupItemsResult.data || {};
        ...
        try {
            ...
            this.modal.setState({
                ...
                content: CustomerSelection.make(
                    null,
                    { paginator: customers, query: q, dealers, countries },
                    false
                ),
                open: true,
            });
    ...
    _pickCustomer({ customer }) {
        ...
        const $btn = document.querySelector(
            "#customer-hook [data-action='open-customer-picker'] span"
        );
    ...
    _pickDealer({ dealer }) {
        ...
        const $btn = document.querySelector(
            "#dealer-hook [data-action='open-dealer-picker'] span"
        );
